...
class DB
{
    ...
    public function log_db_errors($error, $query)
    {
        ...
        $my_array = array();
        $my_array['error'] = $error;
        $my_array['details'] = htmlentities($query);
        $my_array['created_at'] = date('Y-m-d H:i:s');
        if(defined('ERROR_LOGGING')){
            $this->insert('error_logs',$my_array);
        }
    ...
    public function log_email($status,$details)
    {


        $my_array = array();
        $my_array['details'] = htmlentities($details);
        $my_array['created_at'] = date('Y-m-d H:i:s');
        $my_array['status'] = $status;
        $this->insert('email_logs',$my_array);
    ...
    public function fetch_object($query, $object = false)
    {
        self::$counter++;
        $row = $this->link->query($query);
        if ($this->link->error) {
    ...
    public function runQuery($query, $object = false)
    {
        self::$counter++;
        //Overwrite the $row var to null
        $row = null;
     ...
     *
     */
    public function special_insert($table, $variables = array())
    {
        self::$counter++;
     ...
     *
     */
    public function special_update($table, $variables = array(), $where = array(), $limit = '')
    {
        self::$counter++;
    ...
    function query_for_special_insert($sql)
    {
    ...
    function update($table, $DataArray, $updateOnField, $updateOnFieldValue, $printSQL = false, $keep_tags = '', $remove_tags = '', $con = '')
    {
        if (count($DataArray) == 0) {
            die($this->log_db_errors($DataArray, "UPDATE statement has not been created"));
        }
        $sql = "UPDATE " . $table . " SET ";
        foreach ($DataArray as $key => $val) {
            $strFields = "`" . $key . "`";
